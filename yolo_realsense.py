# yolo test rotation, distance 완성되면 여기에 합치기
...
'''
# 이미지 저장 함수
def save_image(image, folder="yoloyoloyoloyolo"):
    if not os.path.exists(folder):
        os.makedirs(folder)
    filename = f"241126_yolo_{int(time.time())}.jpg"
...
def mac_camera():
    ...
    while True:
        ...
        # frame 출력
        cv.imshow("YOLOv8-realsense Detection", annotated_frame)
